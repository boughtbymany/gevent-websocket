...
class WebSocketHandler(WSGIHandler):
    ...
    def upgrade_connection(self):
        ...
        try:
            # python2
            _hash = base64.b64encode(hashlib.sha1(key + self.GUID).digest())
        except:
            # python3
            _hash = base64.b64encode(hashlib.sha1((key + self.GUID).encode()).digest()).decode()

        headers = [
            ("Upgrade", "websocket"),
            ("Connection", "Upgrade"),
            ("Sec-WebSocket-Accept", _hash),
        ]
