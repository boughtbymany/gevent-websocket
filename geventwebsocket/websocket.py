...
class WebSocket(object):
    ...
    def send_frame(self, message, opcode):
        ...
        header = Header.encode_header(True, opcode, 0, len(message), '')
    ...
    def close(self, code=1000, message=''):
        ...
        try:
            ...
            _ = struct.pack('!H%ds' % len(message), code, message)

            self.send_frame(_, opcode=self.OPCODE_CLOSE)
        except WebSocketError:
...
class Header(object):
    ...
    def mask_payload(self, payload):
        ...
        for i in six.moves.xrange(self.length):
            payload[i] ^= mask[i % 4]
    ...
    @classmethod
    def encode_header(cls, fin, opcode, flags, length, mask):
        """
        ...
        return six.b(chr(first_byte) + chr(second_byte) + extra.decode('utf-8'))
